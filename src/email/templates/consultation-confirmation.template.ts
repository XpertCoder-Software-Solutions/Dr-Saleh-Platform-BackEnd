...
export type ConsultationConfirmationTemplateParams = {
  brandName: string;
  fullName: string;
  logoUrl?: string;
  platformUrl?: string;
...
export function buildConsultationConfirmationTemplate({
  brandName,
  fullName,
  logoUrl,
  platformUrl,
  supportEmail,
}: ConsultationConfirmationTemplateParams): string {
...
<html lang="ar" dir="rtl">
  <head>
    ...
    <title>تم استلام طلب الاستشارة</title>
  </head>
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:18px;overflow:hidden;border:1px solid #d9e6f5;box-shadow:0 14px 40px rgba(0,33,75,0.12);">
      ...
      <tr>
        <td style="padding:28px 24px 12px;">
          <h1 style="margin:0 0 12px;font-size:24px;line-height:1.4;color:#111827;">تم استلام طلب الاستشارة</h1>
          <p style="margin:0;color:#374151;font-size:15px;line-height:1.9;">مرحباً <strong>${safeFullName}</strong>،</p>
          <p style="margin:12px 0 0;color:#374151;font-size:15px;line-height:1.9;">تم استلام طلب الاستشارة الخاص بكم بنجاح، وسيتم مراجعة الطلب والتواصل معكم في أقرب وقت ممكن.</p>
        </td>
      </tr>
      <tr>
        <td style="padding:12px 24px 4px;">
          <div style="border:1px solid #b9d7f2;background:linear-gradient(180deg,#f8fcff 0%,#eef6ff 100%);border-radius:14px;padding:16px;">
            <p style="margin:0;color:#111827;font-size:15px;line-height:1.9;">نشكركم على ثقتكم في <strong>${safeBrandName}</strong>. سيقوم فريقنا بمراجعة تفاصيل الطلب بعناية ثم التواصل معكم عبر بيانات الاتصال التي تم إدخالها.</p>
          </div>
        </td>
      </tr>
      <tr>
        <td style="padding:18px 24px 8px;">
          ${openPlatformButton}
          ${supportSection}
          <p style="margin:18px 0 0;color:#374151;font-size:14px;line-height:1.9;">فريق منصة د. صالح</p>
        </td>
      </tr>
      <tr>
