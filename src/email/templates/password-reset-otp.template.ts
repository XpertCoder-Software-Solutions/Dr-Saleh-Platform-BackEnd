...
<html lang="en">
  <head>
    ...
    <title>${safeBrandName} Password Reset OTP</title>
  </head>
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="max-width:640px;margin:0 auto;background:#ffffff;border-radius:18px;overflow:hidden;border:1px solid #d9e6f5;box-shadow:0 14px 40px rgba(0,33,75,0.12);">
      ...
      <tr>
        <td style="padding:28px 24px 12px;">
          ...
          <p style="margin:0;color:#374151;font-size:15px;line-height:1.7;">Hello <strong>${safeFullName}</strong>, we received a password reset request for your <strong>${safeBrandName}</strong> account.</p>
          <p style="margin:12px 0 0;color:#374151;font-size:15px;line-height:1.7;">Use the one-time code below to choose a new password:</p>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        <td style="padding:18px 24px 8px;">
          ${openPlatformButton}
          <p style="margin:16px 0 0;color:#6b7280;font-size:12px;line-height:1.7;">If you did not request a password reset, you can safely ignore this email.</p>
          ${supportSection}
        </td>
      </tr>
      <tr>
